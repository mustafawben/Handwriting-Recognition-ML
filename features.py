...
def projectOntoPC(X, pcs, n_components):
    ...
    #assert temp.shape == (X.shape[0], n_components)
    
    centered = centerData(X)
    n_features = pcs[:,0:n_components]
    return np.matmul(centered,n_features)
    ...
    for i in range(d+1):
        for j in range(i, d+1):
            for k in range(j, d+1):
                if ((i != j) + ( j != k) + ( k != i) == 3):
                    newData[:, featureCount] = np.sqrt(6)*(X_withones[:,i] * X_withones[:,j] * X_withones[:,k])
                elif ((i != j) + ( j != k) + ( k != i) == 2):
                    newData[:, featureCount] = np.sqrt(3)*(X_withones[:,i] * X_withones[:,j] * X_withones[:,k])
                else:
                    newData[:, featureCount] = (X_withones[:,i] * X_withones[:,j] * X_withones[:,k])
                featureCount += 1
    return newData
